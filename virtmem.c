...
int fifo_adress = 0; //used to keep track of the first page added to the page table
int clock_hand  = 0; //used to keep track of the clock hand
...
typedef struct page_table_entry page;
struct page_table_entry {
    long page_num;
    int dirty;
    int ref;
    int free;
    struct page_table_entry *next;//So that the page_table_entry can be used in a linked list
    struct page_table_entry *prev;
};

page *lru_head = NULL;//For a queue keeping track of lru priority
page *lru_tail = NULL;
 ...
 */
void enqueue(page *cur){//enqueue for a doubly linked list
    if(lru_head == NULL){
        lru_head = cur;
    } else {
        lru_tail->next = cur;
...
page *dequeue(){ //Doubly linked list dequeue
    page *cur;
...
long resolve_address(long logical, int memwrite)
{
    ...
    if (frame != -1) {
        //If we are using LRU and the page is already in the table and needs to be used then add it to the end of the LRU queue from where it currently is in the queue
        if(page_replacement_scheme == REPLACE_LRU){
            ...
            if(page_table[frame].prev != NULL){
                ...
                lru_head = page_table[frame].next;
            }

            page_table[frame].next = NULL;
            page_table[frame].prev = NULL;

            enqueue(&page_table[frame]);
        } else if (page_replacement_scheme == REPLACE_CLOCK){//If we are using clock algorithm set reference bit to 1
            page_table[frame].ref = 1;
        }

        if(memwrite){ //If we are writing to and the page is already in the page table dirty bit is true
    ...
    if (frame != -1) {
        ...
        if(page_replacement_scheme == REPLACE_LRU){ //If we are using lru enqueue the page to the lru queue
            enqueue(&page_table[frame]);
        } 
        return effective;
    } else {
        if(page_replacement_scheme == REPLACE_FIFO){ //Replacing frame using fifo
            if(page_table[fifo_adress].dirty){//If it is dirty perform swap out
                swap_outs ++;
                page_table[fifo_adress].dirty = FALSE;
            }
            page_table[fifo_adress].page_num = page;
            page_table[fifo_adress].free = FALSE;
            swap_ins ++;
            
            effective = (fifo_adress << size_of_frame) | offset;
            fifo_adress = (fifo_adress + 1) % size_of_memory;
            
            return effective;
        } else if ( page_replacement_scheme == REPLACE_LRU){ //Replacing a frame using lru algorithm
            if(lru_head->dirty){//If it is dirty perform swap out
                swap_outs ++;
                lru_head->dirty = FALSE;
            }
            
            lru_head->page_num = page;

            enqueue(dequeue());
            swap_ins ++;
            
            for(int i = 0; i < size_of_memory; i ++){
                if(&page_table[i] == lru_tail){
                    effective = (i << size_of_frame) | offset;
                    return effective;
                }
            }
            
        } else if(page_replacement_scheme == REPLACE_CLOCK){ //replacing frame using clock
            while(page_table[clock_hand].ref == 1){ //finding the page to replace
                page_table[clock_hand].ref = 0;
                clock_hand = (clock_hand + 1) % size_of_memory;
            }
            if(page_table[clock_hand].dirty == TRUE){ //If it is dirty perform swap out
                swap_outs++;
                page_table[clock_hand].dirty = FALSE;
            }

            page_table[clock_hand].page_num = page;
            page_table[clock_hand].ref = 1;
            swap_ins ++;

            effective = (clock_hand << size_of_frame) | offset;
            return effective;
